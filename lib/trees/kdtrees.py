...
def KDTrees(point_clouds, dim=3, steps=5, lim=1, det=False, gamma=10., medians=False):
    ...
    while min_nide_size > lim:
        ...
        if medians:
            m = np.empty((len(point_clouds), 2**step), dtype=np.float32)
        ...
        for i, subsrt in enumerate(srt0):
            ...
            ns[:, :, i] = norms.copy()
            if medians:
                m[:, i] = crd[np.arange(len(crd)), split_ax, srt_buf[:, srt_buf.shape[1]/2]].copy()
