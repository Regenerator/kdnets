...
def prepare_shapenet(path2data, path2save, pose='normal'):
	...
	train_nFaces = hf.create_dataset('train_nFaces', (len(train_filenames),), dtype=np.int32)
	...
	val_nFaces = hf.create_dataset('val_nFaces', (len(val_filenames),), dtype=np.int32)
	...
	test_nFaces = hf.create_dataset('test_nFaces', (len(test_filenames),), dtype=np.int32)
	...
	for i, shapefile in enumerate(train_filenames):
	    ...
	    train_nFaces[i] = faces_pos + faces_offset
	...
	for i, shapefile in enumerate(val_filenames):
	    ...
	    val_nFaces[i] = faces_pos + faces_offset
	...
	for i, shapefile in enumerate(test_filenames):
	    ...
	    test_nFaces[i] = faces_pos + faces_offset
	    
	    vertices_pos += vertices_offset
	    faces_pos += faces_offset

	hf.close()
	print('Data in ' + path2data + ' ('+ pose + ') ' + 'is processed and saved to ' + path2save + '/shapenet_' + pose + '.h5')