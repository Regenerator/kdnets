...
def prepare_modelnet40(path2data, path2save):
	...
	for i, cl in enumerate(classes):
	    ...
	    for j, shapefile in enumerate(cl_train_files):
	        ...
	        train_nFaces = np.hstack((train_nFaces, numFaces if i + j == 0 else numFaces + train_nFaces[-1]))
	    ...
	    for j, shapefile in enumerate(cl_test_files):
	        ...
	        test_nFaces = np.hstack((test_nFaces, numFaces if i + j == 0 else numFaces + test_nFaces[-1]))
	...
	print('Data in ' + path2data + ' is processed and saved to ' + path2save + '/modelnet40.h5')